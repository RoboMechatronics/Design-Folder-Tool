...
prelim_folder_path = paths[10]
Template_for_folder=paths[11]
#---------------------------------------------------------------------------------------------------#
design_folder_path = design_folder_path[len('DESIGN_FOLDER_PATH')+1:].replace("\n", "")
folder_template_path = folder_template_path[len('FOLDER_TEMPLATE_PATH')+1:].replace("\n", "")
ECO_folder_template_name = ECO_folder_template_name[len('ECO_FOLDER_TEMPLATE_NAME')+1:].replace("\n", "")
new_folder_template_name = new_folder_template_name[len('NEW_FOLDER_TEMPLATE_NAME')+1:].replace("\n", "")
external_issue_folder_path = external_issue_folder_path[len('EXTERNAL_ISSUE_FOLDER_PATH')+1:].replace("\n", "")
external_issue_folder_path = external_issue_folder_path.replace("username", os.environ.get('USERNAME'))
prelim_folder_path = prelim_folder_path[len('DESIGN_FOLDER_PATH')+1:].replace("\n", "")
Template_for_folder = Template_for_folder[len("Template_for_folder")+1:].replace("\n","")
__build__ = True
#--------------------------------------------------------------------------------------------------------------------------#
# Start App class
class App(QWidget):
    ...
    def initUI(self):
        ...
        #prelim_folder_link_label = QLabel(prelim_folder_path, self)
        #prelim_folder_link_label.setFont(QFont('Arial', 8))
        #prelim_folder_link_label.setStyleSheet(
        #    "color: rgb(255,255,255); background-color: rgba(0, 0, 0, 0)")
        ...
        #prelim_folder_link_label.mousePressEvent = self.Open_Parent_Prelim_Folder
        ...
        if __build__ == True:
            ...
            Create_button.setIcon(
                QIcon('my_app_venv_01/app/dist/icon/create.png'))
            Create_ECO_button.setIcon(
                QIcon('my_app_venv_01/app/dist/icon/create_eco.png'))
            Open_Latest_ECO_button.setIcon(
                QIcon('my_app_venv_01/app/dist/icon/find_eco.png'))
        #------------------------------------------------------------------#
        # Window Title Style Sheet
        self.setStyleSheet("background-color: rgba(10, 20, 10, 0.5);")
        ...
        self.listwidget.setStyleSheet(
            "color: rgb(255,255,255); border-radius: 10px; background-color : rgba(255, 255, 255, 0.1);")
        ...
        self.combobox.addItems(['Some folder links here!',Template_for_folder, design_folder_path, prelim_folder_path])
        ...
        #vbox.addWidget(prelim_folder_link_label)
        ...
        if self.part_number_textbox.text() != "":  # if the part number is available
            ...
            # If folder don't exist, runing Advanced Search mode
                ...
                if len(list_result) < 1:  # is empty
                    self.message.setText("Folder doesn't exist")

                if len(list_result) == 1 and list_result[0].find(".") == 0:  # has only a folder
                    Open_Folder2(design_folder_path, list_result[0])

                if len(list_result) >= 1 and list_result[0].find(".") != 0:  # is not empty
    ...
    def Open_ECO_Folder(self, eco_number=""):
        ...
        if status == True:
            ...
            if count == True:
                ...
                self.message.setText("The ECO number doesn't exist")
                return 0
    ...
    def Open_Latest_ECO_Folder(self):
        ...
        if part_number != "":
           # count = False
            ...
            if temp_number_list[0] == "":
                temp_number_list[0] = 0

            max = int(temp_number_list[0])
            for i in range(1, len(temp_number_list)):
                if temp_number_list[i] == "":
                    temp_number_list[i] = 0
                if max < int(temp_number_list[i]):
                    max = int(temp_number_list[i])
    ...
    # def Open_Parent_Prelim_Folder(self, event):
    #     self.message.setText("")
    #     os.startfile(prelim_folder_path, 'open')

    def Combobox_selected(self, event):

        self.message.setText("")
        if os.path.exists(self.combobox.currentText()) == 1:  # existed
            os.startfile(self.combobox.currentText(), 'open')
    ...
    def Create_Folder(self):
        ...
        if self.part_number_textbox.text() != "":
            ...
            if os.path.exists(path) == 0:  # not existed
                ...
                self.message.setText("Folder exists, please check again!")
        else:
            #self.MsgBox("Warning!", "Please type Part Number!", 1)
            self.message.setText("Please type Part Number!")
    ...
    def Open_Prelim_Folder(self):
        ...
        if self.part_number_textbox.text() != "":  # if the part number is available
            ...
            # Get  full path of folder
            path = prelim_folder_path + chr(92) + part_number
            ...
            # If folder don't exist, runing Advanced Search mode
                ...
                list_result = AdvancedSearch(
                    self.part_number_textbox.text(), prelim_folder_path)  # result a list type
                ...
                if len(list_result) == 1:  # has only a folder
                    Open_Folder2(prelim_folder_path, list_result[0])
    ...
    def keyPressEvent(self, event):
        ...
        if event.modifiers() == Qt.ControlModifier and event.key() == Qt.Key_E and self.part_number_textbox.text() != "":
            #self.ECO_part_number_textbox.setText("")
            self.Open_External_Issue_Folder()

        if event.modifiers() == Qt.ControlModifier and (event.key() == Qt.Key_I) and self.part_number_textbox.text() != "":
            self.Open_Prelim_Folder()
            #self.Open_Prelim_Folder(self.part_number_textbox.text())

        if (event.key() == Qt.Key_Enter or event.key() == Qt.Key_Return) and self.part_number_textbox.text() != "":
            #self.ECO_part_number_textbox.setText("")
            self.Open_Folder()
...
def AdvancedSearch(name, folder_path):
    ...
    if os.path.exists(folder_path) != 1:
        return []
...
def Remove_file(path):
    ...
    for root, dirs, files in os.walk(path): 
        for name in files: 
            if name.upper().find("Thumbs.db".upper())>=0:
